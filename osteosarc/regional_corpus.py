"""Historical regional corpus format and nested manifests from OpenVax Topiary.

The format preserves reviewed source/reference/translation relationships. No
prediction or annotation runs during transport. Original: Apache-2.0 Topiary.
"""
...
from .bundles import _publication, safe_path
...
def _generate_regional_corpus(recipe_path, destination, *, cache=None, source_directory=None,
    ...
    for dataset in manifest["datasets"].values():
        target = safe_path(indels, dataset["file"])
    ...
    for asset in manifest["assets"]:
        path = safe_path(source, asset["filename"])
    ...
    for group, filename, receipt_name in (
        ...
        for name in files:
            files[name] = digest(safe_path(root, name))
        write_json(path, manifest)
...
def generate_regional_corpus(recipe_path, destination, **kwargs):
    """Atomically export the pinned regional corpus without a sibling checkout."""
    with _publication(destination) as work:
        # The legacy writer expects to create its destination.
        inner = work / "corpus"
        result = _generate_regional_corpus(recipe_path, inner, **kwargs)
        for path in list(inner.iterdir()):
            path.rename(work / path.name)
        inner.rmdir()
        budget = json.loads(Path(recipe_path).read_text()).get("size_budget_bytes", 64 * 1024 * 1024)
        if sum(p.stat().st_size for p in work.rglob("*") if p.is_file()) > budget:
            raise ValueError("Regional corpus exceeds size budget")
        return result
